...
const TextFieldConfig = forwardRef((props, ref) => {
    ...
    const [config, setConfig] = useState({
        ...
        defaultType: 'custom', // custom, quick, formula, linkage
        defaultValue: '',
        clearButton: true,
        showCounter: false,
        required: false,
    ...
    useImperativeHandle(ref, () => ({
        ...
        getConfig: () => {
            ...
            if (enabledProps.clearButton) finalConfig.hasClear = config.clearButton;
            if (enabledProps.showCounter) finalConfig.hasLimitHint = config.showCounter;
            ...
            if (enabledProps.minLength && config.minLength) finalConfig.minLength = parseInt(config.minLength, 10);
            if (enabledProps.maxLength && config.maxLength) finalConfig.maxLength = parseInt(config.maxLength, 10);
        ...
        importConfig: (schema) => {
            ...
            // 清除按钮
            if (props.hasClear !== undefined) {
                newConfig.clearButton = props.hasClear;
                newEnabled.clearButton = true;
            }
            // 显示计数器
            if (props.hasLimitHint !== undefined) {
                newConfig.showCounter = props.hasLimitHint;
                newEnabled.showCounter = true;
            }
            // 长度限制
            if (props.minLength !== undefined) {
                newConfig.minLength = props.minLength;
                newEnabled.minLength = true;
            }
            if (props.maxLength !== undefined) {
                newConfig.maxLength = props.maxLength;
                ...
                <div>
                    ...
                    <ConfigRow label="默认值" propKey="defaultValue" enabledProps={enabledProps} onToggleProp={handleToggleProp}>
                        ...
                        >
                            ...
                            <option value="quick">快捷配置</option>
                    ...
                    {/* 清除按钮 */}
                    <ConfigRow label="清除按钮" propKey="clearButton" enabledProps={enabledProps} onToggleProp={handleToggleProp}>
                        <div style={{ paddingTop: '6px' }}>
                            <Switch checked={config.clearButton} onChange={(val) => updateConfig('clearButton', val)} />
                        </div>
                    </ConfigRow>

                    {/* 显示计数器 */}
                    <ConfigRow label="显示计数器" propKey="showCounter" enabledProps={enabledProps} onToggleProp={handleToggleProp}>
                        <div style={{ paddingTop: '6px' }}>
                            <Switch checked={config.showCounter} onChange={(val) => updateConfig('showCounter', val)} />
                        </div>
                    </ConfigRow>
